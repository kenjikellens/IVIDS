    ...
    try {
        ...
        if (genreSelectBtn && genreModal) {
            genreSelectBtn.addEventListener('click', () => {
                ...
                // Focus first genre or done button
                const firstGenre = genreModal.querySelector('input');
                if (firstGenre) {
                    SpatialNav.setFocus(firstGenre.closest('.focusable'));
                } else if (genreDoneBtn) {
                    SpatialNav.setFocus(genreDoneBtn);
                }
            });
            ...
            const closeModal = () => {
                ...
                // Perform search on close if needed, or it happens on change
            };

            if (genreDoneBtn) {
                genreDoneBtn.addEventListener('click', closeModal);
            }

            // Handle Back key to close modal
            const originalOnBack = SpatialNav.onBack;
            SpatialNav.onBack = () => {
                if (genreModal.classList.contains('active')) {
                    closeModal();
...
function initInfiniteScroll() {
    try {
        const scrollContainer = document.getElementById('main-view');
        ...
        scrollListener = () => {
            try {
                ...
                timeout = setTimeout(() => {
                    ...
                    // Trigger load when within 100px of bottom (less aggressive)
        ...
        if (genreContainer) {
            try {
                ...
                genreContainer.innerHTML = genres.map(genre => {
                    const isSelected = currentFilters.genres.includes(genre.id);
                    return `
                    <label class="filter-checkbox focusable ${isSelected ? 'selected' : ''}" tabindex="0">
                        <input type="checkbox" value="${genre.id}" data-type="genre" ${isSelected ? 'checked' : ''}> ${genre.name}
                    </label>
                `}).join('');
            } catch (genreError) {
        ...
        if (certContainer) {
            try {
                ...
                certContainer.innerHTML = certs.map(cert => {
                    const isSelected = currentFilters.certification === cert;
                    return `
                    <label class="filter-checkbox focusable ${isSelected ? 'selected' : ''}" tabindex="0">
                        <input type="radio" name="certification" value="${cert}" data-type="cert" ${isSelected ? 'checked' : ''}> ${cert}
                    </label>
                `}).join('');
            } catch (certError) {
...
function handleFilterChange(e) {
    try {
        ...
        const label = target.closest('.filter-checkbox');
        ...
        if (name === 'media-type') {
            ...
            // Clear other certs visual state
            document.querySelectorAll('input[name="certification"]').forEach(el => {
                const l = el.closest('.filter-checkbox');
...
function updateGenreButton() {
    const countSpan = document.getElementById('genre-count');
    if (countSpan) {
...
async function fetchResults(reset) {
    try {
        ...
        if (reset) {
            grid.innerHTML = `<div class="loading">${getLoaderHtml()}</div>`;
        }
