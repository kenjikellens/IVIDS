export const SpatialNav = {
    ...
    focusTrapContainer: null, // If set, navigation is limited to this container
    ...
    setFocusTrap(container) {
        ...
        if (container) {
            // Find first focusable inside trap
            const first = container.querySelector(this.focusableSelector);
    ...
    focusFirst() {
        // If trapped, only look inside trap
        const scope = this.focusTrapContainer || document;
        const elements = scope.querySelectorAll(this.focusableSelector);
        ...
        for (const id of heroButtons) {
            const heroBtn = scope.getElementById ? scope.getElementById(id) : scope.querySelector(`#${id}`);
            if (heroBtn) {
        ...
        const searchBtn = scope.getElementById ? scope.getElementById('search-btn') : scope.querySelector('#search-btn');
        if (searchBtn) {
        ...
        const playlistNavBtn = scope.querySelector('[data-route="playlists"]');
        ...
        const languageSelect = scope.getElementById ? scope.getElementById('language-select') : scope.querySelector('#language-select');
        if (languageSelect) {
            this.setFocus(languageSelect);
            return;
        }

        // Priority 5: Other content elements (skip nav-items if possible, unless only nav items exist)
    ...
    setFocus(element) {
        ...
        if (element.closest('.hero')) {
            ...
            try {
                ...
                // Fallback for older browsers/TVs
    ...
    handleKey(e) {
        ...
        // If trapped, ensure current is inside trap
        if (this.focusTrapContainer && !this.focusTrapContainer.contains(current)) {
            this.focusFirst();
            return;
        }
        ...
        switch (keyCode) {
            ...
            case 8: // Backspace (PC) / Escape
                // Allow Backspace to delete characters in inputs
    ...
    findNext(current, direction) {
        // Scope search to trap container if set
        const scope = this.focusTrapContainer || document;

        // --- Search Page Overrides ---
        if (current.id === 'search-input') {
            if (direction === 'right') return scope.querySelector('#search-btn');
            if (direction === 'left') return scope.querySelector('.nav-item[data-route="search"]');
            if (direction === 'down') return scope.querySelector('#year-filter');
        }
        if (current.id === 'search-btn') {
            if (direction === 'left') return scope.querySelector('#search-input');
            if (direction === 'right') return scope.querySelector('#view-toggle-btn');
        }
        ...
        if (current.id === 'sort-by') {
            if (direction === 'right') return scope.querySelector('#year-filter');
            if (direction === 'up') return scope.querySelector('#search-input');
        }

        if (current.id === 'year-filter') {
            if (direction === 'up') return scope.querySelector('#search-input');
            if (direction === 'left') return scope.querySelector('#sort-by');
            if (direction === 'right') return scope.querySelector('input[value="movie"]')?.parentElement;
        }

        // Type Filter (Movie & Series)
        const movieLabel = scope.querySelector('input[value="movie"]')?.parentElement;
        const seriesLabel = scope.querySelector('input[value="tv"]')?.parentElement;

        if (movieLabel && current === movieLabel) {
            if (direction === 'left') return scope.querySelector('#year-filter');
            if (direction === 'right') return scope.querySelector('#genre-select-btn');
            if (direction === 'up') return scope.querySelector('#search-input');
            if (direction === 'down') return seriesLabel;
        }

        if (seriesLabel && current === seriesLabel) {
            if (direction === 'right') return scope.querySelector('#genre-select-btn');
            if (direction === 'up') return movieLabel;
        }

        // Genre Button
        if (current.id === 'genre-select-btn') {
            if (direction === 'left') return movieLabel;
            if (direction === 'right') return scope.querySelector('#cert-filters .filter-checkbox');
            if (direction === 'up') return scope.querySelector('#search-input');
        }

        // Age Rating (First item)
        const firstCert = scope.querySelector('#cert-filters .filter-checkbox');
        if (firstCert && current === firstCert) {
            if (direction === 'left') return scope.querySelector('#genre-select-btn');
            if (direction === 'up') return scope.querySelector('#search-input');
        }
        // -----------------------------
        ...
        const all = Array.from(scope.querySelectorAll(this.focusableSelector));
